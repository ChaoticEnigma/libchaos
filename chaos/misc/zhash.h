...
#define ZHASH_HAS_MD5   (defined(LIBCHAOS_HAS_CRYPTO) || LIBCHAOS_PLATFORM == LIBCHAOS_PLATFORM_WINDOWS)
#define ZHASH_HAS_SHA1  (defined(LIBCHAOS_HAS_CRYPTO) || LIBCHAOS_PLATFORM == LIBCHAOS_PLATFORM_WINDOWS)
...
public:
    ...
    ~ZHashMethod(){ if(_context != nullptr) md5_finish(_context); }
protected:
    void feedHash(const zbyte *data, zu64 size){
        if(_context != nullptr)
...
public:
    ...
    ~ZHashMethod(){ if(_context != nullptr) sha1_finish(_context); }
protected:
    void feedHash(const zbyte *data, zu64 size){
        if(_context != nullptr)
